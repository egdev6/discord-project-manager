...
2. Use `docs/architecture/openclaw-global-channel-guides.md` and `examples/openclaw-global-channel-guides.fake.yaml` as the canonical source for reserved `OpenClaw Global` control-channel topics and starter guidance.
3. Use `docs/architecture/discord-semantic-channel-guides.md` and `examples/discord-semantic-channel-guides.fake.yaml` as the canonical source for managed Project Manager channel topics and starter/pinned prompts.
4. Use `docs/architecture/discord-project-manager-global-init.md` and `examples/discord-project-manager-global-init.fake.yaml` when reviewing the fake `/project-manager init` global category setup contract.
5. Use `docs/architecture/discord-project-manager-project-create.md` and `examples/discord-project-manager-project-create.fake.yaml` when reviewing the fake `/project create` per-project category setup contract.
6. Use `docs/architecture/discord-project-manager-project-delete.md` and `examples/discord-project-manager-project-delete.fake.yaml` when reviewing the fake `/project delete` preview/tombstone contract.
7. Use `docs/architecture/discord-channel-scaffolding-status-repair.md` and `examples/discord-channel-scaffolding-status-repair.fake.yaml` when reviewing fake status and repair previews for missing, renamed, or unmanaged managed scaffolding.
8. Use `docs/architecture/discord-managed-channel-routing.md` and `examples/discord-managed-channel-routing.fake.yaml` when reviewing persisted semantic metadata routing for managed Project Manager channels.
9. Use `docs/architecture/discord-general-advisor.md` and `examples/discord-general-advisor.fake.yaml` when reviewing response-only routing and operational architecture advice.
10. Use `docs/architecture/discord-admin-ux.md` and `examples/discord-admin-ux.fake.yaml` when reviewing fake admin UX flows for profiles, scope bindings, effective resolver inspection, capabilities, and backup/export requests.
11. Install the external OpenClaw Discord plugin before live validation.
12. For current Project Manager flows, resolve managed channels from persisted semantic metadata/IDs.
13. Use `<network-slug>-<project-slug>` only when explicitly validating the legacy fallback route.
14. Keep real guild and channel IDs outside the repo.
15. Route channel-local memory by raw Discord IDs.
16. Route durable project reads by approved project/network slugs.
17. Ask for human approval before durable project writes.
18. Load `skills/discord-approval-gate/SKILL.md` for Discord write-like intents before any persistence.
19. Use `docs/operations/discord-approval-responses.md` for approval prompts and audit trail requirements.
20. Use `docs/architecture/channel-context-namespace-mapping.md` and `examples/discord-channel-context.fake.yaml` as the resolver reference and fake fixture.
21. Use `docs/architecture/discord-topology-reconciliation.md` and `examples/discord-topology-reconciliation.fake.yaml` when validating category/channel discovery before provisioning.
22. Use `docs/architecture/discord-context-namespace-provisioning.md` and `examples/discord-context-provisioning.fake.yaml` when reviewing approved draft context artifacts.
...
The canonical resolver contract lives in `docs/architecture/channel-context-namespace-mapping.md`, and `examples/discord-channel-context.fake.yaml` provides fake matched/unmapped fixtures. For reserved `OpenClaw Global` control-channel copy, use `docs/architecture/openclaw-global-channel-guides.md` and `examples/openclaw-global-channel-guides.fake.yaml`; validate with `bash scripts/validate-openclaw-global-channel-guides.sh`. For managed global/project scaffolding copy, use `docs/architecture/discord-semantic-channel-guides.md` and `examples/discord-semantic-channel-guides.fake.yaml` instead of duplicating topics or starter prompts in handlers. For global Project Manager initialization, use `docs/architecture/discord-project-manager-global-init.md` and `examples/discord-project-manager-global-init.fake.yaml` as the fake-first command/topology contract. For per-project category creation, use `docs/architecture/discord-project-manager-project-create.md` and `examples/discord-project-manager-project-create.fake.yaml` as the fake-first project creation contract. For safe project deletion, use `docs/architecture/discord-project-manager-project-delete.md` and `examples/discord-project-manager-project-delete.fake.yaml`; validate with `bash scripts/validate-discord-project-manager-project-delete.sh`. For status and repair previews over managed scaffolding drift, use `docs/architecture/discord-channel-scaffolding-status-repair.md` and `examples/discord-channel-scaffolding-status-repair.fake.yaml` before proposing any recreate or metadata refresh. For managed channel prompt routing, use `docs/architecture/discord-managed-channel-routing.md` and `examples/discord-managed-channel-routing.fake.yaml` so scope/field decisions come from persisted semantic metadata, not channel names. For general/help-channel advice about where a request belongs, use `docs/architecture/discord-general-advisor.md`, `examples/discord-general-advisor.fake.yaml`, and `scripts/validate-discord-general-advisor.sh`; the advisor recommends routes and copyable handoff text but does not execute writes. For admin UX previews, use `docs/architecture/discord-admin-ux.md`, `examples/discord-admin-ux.fake.yaml`, and `scripts/validate-discord-admin-ux.sh`; admin UX fixtures must keep private profile content out of repo artifacts.
...
bash scripts/validate-discord-project-manager-project-delete.sh
bash scripts/validate-discord-project-manager-global-init.sh
bash scripts/validate-discord-semantic-channel-guides.sh
bash scripts/validate-openclaw-global-channel-guides.sh
