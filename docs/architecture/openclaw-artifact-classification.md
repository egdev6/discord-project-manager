...
External-service targets such as Buffer, Discord, browser sessions, publishing platforms, and analytics providers need more than approval gating before they become executable runtime behavior.

Before any external connector action moves beyond fake-first contract status, the implementation must define:

- idempotency key or duplicate-submission guard;
- retry/backoff boundary for rate limits and transient provider errors;
- auth-expiry handling and safe reauthorization path;
- partial-success reconciliation for publish/schedule/update operations;
- manual recovery path when the provider state is unknown.

Until those rules exist for a connector, classify connector changes as proposal/config work only and keep `publishing_enabled: false`, `scheduling_enabled: false`, and provider activity disabled in public fixtures.
...
- [ ] SDD development work routes to `runner_backend: gentle-sdd` without making Gentle the Discord-facing authority.
